...
更新时间：2026-09-20（Asia/Shanghai），第 4 周本地主线收尾完成；下一课进入第 5 周只读文件工具的 Workspace 路径边界。
...
- 正式进度：第 1～4 周完成，4 / 12（33.3%）；第 5 周尚未开始。
...
**第 5 周：只读文件工具的 Workspace 路径边界。**

先做一个最小服务：由可信任务归属确定Workspace根目录，解析用户/模型给出的相对路径，拒绝绝对路径、上级逃逸、符号链接逃逸以及未绑定目录；暂不引入Shell、写文件、目录树UI或完整搜索。开始前核对现有目录绑定服务及跨平台路径行为，给出完整可运行参考和必要注释。核心由学习者编写，收到“完成了”后教练补成功/失败及真实文件系统测试。

本次用户代写授权仅限第4周收尾。第5周引入外部进程前必须扩展停止证据与Sandbox，不能直接复用“父进程不存在”作为所有子进程已停止的证明。
