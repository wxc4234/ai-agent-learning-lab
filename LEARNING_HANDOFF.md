...
更新时间：2026-09-16（Asia/Shanghai），Task 创建幂等模型与迁移已完成。

这是新会话的唯一动态进度入口，长期规则见自动加载的 AGENTS.md。详细历史与验证记录按需查看，不再整篇加载计划或课程大纲。

## 2026-09-16 收尾与新会话接续

- 今日已完成：Task 详情/URL 恢复、空任务删除链路与本地会话保护、运行历史列表和详情、侧栏菜单与加载体验、右侧栏调宽、Markdown 回复展示、启动迁移检查、创建幂等模型与迁移。详细验收证据见下文及 ENVIRONMENT.md。
- 当前停在“创建幂等事务服务”：完整参考已在上一会话给出，学习者尚未表示完成；源码仍是原创建服务。不得将已提供参考视为已实现或已验收，也不要提前创建本课测试。
- 新会话继续在本主仓库 main 学习。先读 AGENTS.md 与本文件，核对工作区；若学习者尚未实现，重新展示当前任务完整参考并等待动手。只有明确“完成了”后才检查改动、补测试和验收。
- 事务课设计：服务端规范化标题并计算 SHA-256；请求键按用户/项目作用域查询；沿用项目行锁串行处理创建，与删除保持锁顺序；同键同内容重放、不同内容冲突、删除后拒绝重建；三条记录同事务提交。request_key 暂时可选以兼容旧 HTTP，HTTP/BFF/UI 后续接入。
- 本次收尾提交与推送不代表第 4 周完成，正式进度仍为 3 / 12。
...
- 开发库迁移 head=0a7b64e039cd；已有验证 alembic check 无差异。Python 3.12.13 与环境细节见 ENVIRONMENT.md；跨电脑需各自安装依赖与迁移。
...
模型与迁移已完成。下一课在创建事务中校验请求键、计算规范化请求指纹，处理同键同内容重放、同键不同内容冲突及结果已删除；Task、Conversation 与请求记录同事务提交。先授权再查请求记录，不能靠前端防重或请求键代替资源授权。本课尚未接入 HTTP/BFF/UI，当前创建接口仍无幂等能力。

已提供本课创建幂等事务服务的完整参考，等待学习者实现并明确“完成了”；尚未修改核心或创建测试、开展本课验收。为兼容现有 HTTP 调用，本课 request_key 暂时可选，提供时启用幂等；后续 HTTP 课收紧输入契约。默认由学习者实现核心，之前直接代写授权仅限对应课次。
