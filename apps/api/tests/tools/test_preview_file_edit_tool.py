...
from app.services.workspace.workspace_edit_preview import EditPreviewError, TextEditPreview, MAX_EDIT_TEXT_BYTES
from app.services.workspace.workspace_file import WorkspaceFileError
from app.services.workspace.workspace_file_preview import WorkspaceFileEditPreview
from app.services.workspace.workspace_path import WorkspacePathError
from app.tools import preview_file_edit as adapter
...
TOOL = TOOL_REGISTRY["preview_file_edit"]


def result(truncated=False):
    # 完整修改内容使用独立哨兵，防止无意asdict后泄露到公开结果。
    return WorkspaceFileEditPreview("src/file.txt", "a" * 64,
                                    TextEditPreview("INTERNAL_FULL_CONTENT", 3, 3, "-旧\n+新\n", truncated))
...
def test_text_boundaries(field, value):
    with pytest.raises(ValidationError):
        adapter.PreviewFileEditArguments.model_validate(ARGS | {field: value})


@pytest.mark.parametrize("field", ["context", "user_id", "workspace_id", "task_id", "root_path", "approved", "baseline_sha256", "max_bytes"])
...
def test_schema_deletion_exact_bytes_and_whitespace():
    ...
    assert adapter.PreviewFileEditArguments(**(ARGS | {"new_text": exact})).new_text == exact
    text = " \r\n\t"
    assert adapter.PreviewFileEditArguments(**(ARGS | {"old_text": text})).old_text == text
...
def test_public_fields_and_context(monkeypatch, truncated):
    ...
    assert json.loads(raw) == {"status": "preview_only", "relative_path": "src/file.txt", "baseline_sha256": "a" * 64,
                               "before_byte_count": 3, "after_byte_count": 3, "diff": "-旧\n+新\n", "diff_truncated": truncated}
...
def test_safe_error_mapping(monkeypatch, error, code):
    def fail(**kwargs):
        raise error
    monkeypatch.setattr(adapter, "preview_task_file_replacement", fail)
    with pytest.raises(SafeToolExecutionError) as caught:
        adapter.preview_file_edit(context=CONTEXT, **ARGS)
...
def test_context_gate_and_visibility(monkeypatch, context):
    monkeypatch.setattr(adapter, "preview_task_file_replacement", lambda **kwargs: pytest.fail("must not execute"))
...
@pytest.mark.parametrize("kind", ["success", "truncated", "safe", "unknown", "invalid", "no-context"])
def test_model_preview_roundtrip(monkeypatch, kind):
    calls = []
    ...
    async def create(**kwargs):
        ...
        if not messages:
            ...
            return build_tool_response(("preview", "preview_file_edit", json.dumps(args)))
        ...
        return build_text_response("仅生成预览，尚未写入")
    ...
    if kind in {"success", "truncated"}:
        ...
        if kind in {"invalid", "no-context"}:
            ...
            assert observation.details == {"safe": "edit_target_ambiguous", "unknown": "RuntimeError"}[kind]
