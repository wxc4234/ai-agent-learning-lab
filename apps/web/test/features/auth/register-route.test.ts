...
const { POST } = await import("../../../src/app/api/auth/register/route.ts");
...
function success(): Response {
    ...
    return Response.json({ external_id: "user-id", username: "中文agent", ignored: PASSWORD }, { headers, status: 201 });
}
...
test("forwards exact credentials, headers and safe identity without issuing cookies", async (t) => {
    ...
    assert.equal(url, "http://backend.test/auth/register");
    assert.ok(received);
...
for (const origin of [null, "null", "https://evil.test", "http://localhost:3000.evil.test", "http://localhost:3001", "https://localhost:3000", "http://localhost:3000/"]) {
    test(`rejects origin ${origin}`, async (t) => {
        ...
        await assertError(await POST(input), 403, "registration_origin_rejected");
...
for (const contentType of [null, "text/plain", "application/x-www-form-urlencoded"]) {
    test(`rejects content type ${contentType}`, async (t) => {
        ...
        await assertError(await POST(input), 415, "unsupported_registration_content_type");
        assert.equal(calls, 0);
    });
}
...
test("rejects malformed JSON before contacting backend", async (t) => {
    let calls = 0;
    t.mock.method(globalThis, "fetch", async () => { calls += 1; return success(); });
    await assertError(await POST(request({}, "{")), 400, "invalid_registration_json");
    assert.equal(calls, 0);
});

for (const [status, code] of [
    [400, "request_rejected"], [409, "username_already_exists"], [403, "registration_origin_rejected"],
    [415, "unsupported_registration_content_type"], [422, "invalid_registration_input"], [500, "registration_failed"],
] as const) {
...
for (const [name, makeResponse, expectedCode] of [
    ["array", () => Response.json([], { status: 201 }), "invalid_backend_response"],
    ["bad identity", () => Response.json({ external_id: 123, username: "test" }, { status: 201 }), "invalid_backend_response"],
    ...
    ["non JSON", () => new Response(PASSWORD, { status: 502 }), "registration_backend_unavailable"],
] as const) {
...
test("connection failures do not expose error messages", async (t) => {
    ...
    await assertError(await POST(request()), 502, "registration_backend_unavailable");
});
...
for (const stage of ["fetch", "body"]) {
    test(`timeout covers ${stage}`, async (t) => {
        ...
        await assertError(await POST(request()), 504, "registration_backend_timeout");
    });
}
...
test("client cancellation propagates to backend", async (t) => {
    ...
    await assertError(await POST(request({}, undefined, client.signal)), 499, "registration_request_cancelled");
});

for (const body of [null, [], {}, { username: "abc" }, { username: 1, password: "password" }, { username: "abc", password: "password", user_id: 1 }]) {
    test(`rejects invalid credentials shape ${JSON.stringify(body)}`, async (t) => {
        const mock = t.mock.method(globalThis, "fetch", async () => success());
        await assertError(await POST(request({}, JSON.stringify(body))), 422, "invalid_registration_input");
