...
const output = '/private/tmp/agent-ui-patch-preview/output/playwright';
await mkdir(output, { recursive: true });
const fixture = await mkdtemp('/private/tmp/agent-patch-preview-');
...
try {
    for (const marker of Object.keys(sources)) {
        const answer = marker === 'conflict' ? '预览失败：补丁上下文与当前文件不一致，请重新生成预览'
            : `仅生成预览，尚未写入${marker === 'truncated' ? '，Diff不完整' : ''}`;
        const { workspace, task } = await setup(`修改预览-${marker}`, true);
        const { events, runId } = await send(`[patch-preview-${marker}] 生成修改预览`, answer);
        assert.deepEqual(events.filter(e => e.type === 'TOOL_CALL_START').map(e => e.tool_name), ['preview_file_patch']);
        const results = events.filter(e => e.type === 'TOOL_CALL_RESULT');
        const failures = events.filter(e => e.type === 'TOOL_CALL_ERROR');
        if (marker === 'conflict') {
            assert.equal(results.length, 0);
            assert.equal(failures.length, 1);
            assert.equal(failures[0].details, 'patch_context_mismatch');
        } else {
        ...
        await page.getByText('preview_file_patch', { exact: true }).waitFor();
        if (marker !== 'conflict') {
            const result = JSON.parse(results[0].result);
