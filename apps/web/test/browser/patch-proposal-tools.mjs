...
const output = '/private/tmp/agent-ui-patch-proposal/output/playwright';
await mkdir(output, { recursive: true });
const fixture = await mkdtemp('/private/tmp/agent-patch-proposal-');
...
const candidate = marker => marker === 'delete' ? '' : marker === 'truncated' ? 'X'.repeat(20000) + '\n' : sources[marker].replace('old', 'new');
...
try {
    for (const marker of Object.keys(sources)) {
        const failed = ['conflict', 'unconfirmed'].includes(marker);
        const answer = failed ? '提案调用失败，请核对结果，勿自动重试' : '提案已保存，等待审批，文件尚未修改';
        const { workspace, task } = await setup(`修改提案-${marker}`, true);
        const { events, runId } = await send(`[patch-proposal-${marker}] 创建待审批补丁提案`, answer);
        assert.deepEqual(events.filter(e => e.type === 'TOOL_CALL_START').map(e => e.tool_name), ['create_file_patch_proposal']);
        const results = events.filter(e => e.type === 'TOOL_CALL_RESULT');
        const failures = events.filter(e => e.type === 'TOOL_CALL_ERROR');
        if (failed) {
            assert.equal(results.length, 0);
            assert.equal(failures.length, 1);
            assert.equal(failures[0].details, marker === 'conflict' ? 'patch_context_mismatch' : 'proposal_save_unconfirmed');
            assert.equal(await page.getByRole('region', { name: '文件修改提案回执', exact: true }).count(), 0);
            await page.getByText(marker === 'conflict' ? '补丁上下文与当前文件不一致，请重新生成预览' : '提案保存结果未确认，可能已经保存；不要自动重复创建，需先核对保存结果', { exact: false }).first().waitFor();
        } else {
            ...
            assert.equal(result.proposed_sha256, createHash('sha256').update(candidate(marker)).digest('hex'));
        ...
        await page.getByText('create_file_patch_proposal', { exact: true }).waitFor();
        evidence.push({ marker, task_id: task.external_id, run_id: Number(runId), result: results.length ? JSON.parse(results[0].result) : null,
            expected_content: candidate(marker) });
        if (!failed) {
        ...
        for (const width of [1366, 1920]) {
            ...
            await page.screenshot({ path: `${output}/patch-proposal-${marker}-${width}.png` });
        }
        ...
        if (!failed) {
            await page.getByRole('region', { name: '文件修改提案回执', exact: true }).waitFor();
            const historical = page.getByRole('region', { name: '文件修改提案详情', exact: true });
            await historical.getByRole('button', { name: '查看提案详情', exact: true }).click();
            await historical.getByLabel('提案 Diff', { exact: true }).waitFor();
        } else {
            assert.equal(await page.getByRole('region', { name: '文件修改提案回执', exact: true }).count(), 0);
        }
        assert.equal(posts, before);
