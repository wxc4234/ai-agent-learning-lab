...
try {
    ...
    const url = `${base}/?workspace=${workspace.external_id}&task=${task.external_id}`;
    for (const page of [first, second]) {
        await page.bringToFront();
        const loaded = page.waitForResponse(r => r.url().endsWith(`/tasks/${task.external_id}/messages`) && r.status() === 200);
        await page.goto(url);
        await loaded;
    ...
    await second.getByText('该会话仍在执行或收尾，请稍后再试', { exact: true }).waitFor();
    await second.screenshot({ path: `${output}/execution-busy-1366.png` });
    console.log('PASS real concurrent tab receives safe 409 and visible busy message');
    ...
    for (let attempt = 0; attempt < 15; attempt++) {
        const response = await second.request.post(`${base}/api/chat/stream`, {
            headers: { Origin: base }, data: { session_id: task.conversation_id, prompt: '取消后恢复验收' },
        });
    ...
    await second.screenshot({ path: `${output}/execution-recovered-1920.png` });
    console.log('PASS cancellation drains execution, releases slot and persisted history reloads');
    assert.deepEqual(errors, []);
} catch (error) {
    await first.screenshot({ path: `${output}/execution-first-failure.png`, timeout: 5000 }).catch(() => {});
    console.error("PAGE ERRORS", errors);
    await second.screenshot({ path: `${output}/execution-failure.png`, timeout: 5000 }).catch(() => {});
    throw error;
} finally {
    await browser.close();
}
