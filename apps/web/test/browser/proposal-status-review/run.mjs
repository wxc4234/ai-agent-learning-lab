...
const temporary = await mkdtemp(path.join(tmpdir(), 'agent-status-review-'));
...
try {
    // 只编译真实组件与样式，不新增生产测试路由。
    const compiler = webpack({
        mode: 'development', devtool: false, context: root,
        entry: path.join(root, 'test/browser/proposal-status-review/entry.tsx'),
    ...
    server = http.createServer(async (req, res) => {
        // 浏览器漏拦截任何API请求时，夹具直接拒绝，绝不连接实际服务。
        ...
        res.end(filename ? await readFile(path.join(temporary, filename)) : '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>提案与样例状态隔离验收</title><link rel="stylesheet" href="/style.css"></head><body><div id="root"></div><script src="/bundle.js"></script></body></html>');
    });
