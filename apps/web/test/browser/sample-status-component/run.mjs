...
const temporary = await mkdtemp(path.join(tmpdir(), 'agent-sample-status-ui-'));
...
try {
    // 编译真实组件和样式，不给生产Next路由添加测试入口。
    const compiler = webpack({
        mode: 'development', devtool: false, context: root,
        entry: path.join(root, 'test/browser/sample-status-component/entry.tsx'),
    ...
    server = http.createServer(async (req, res) => {
        ...
        res.end(filename ? await readFile(path.join(temporary, filename)) : '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>样例登记状态隔离测试</title><link rel="stylesheet" href="/style.css"></head><body><div id="root"></div><script src="/bundle.js"></script></body></html>');
    });
