    ...
    return (
        <div className="mt-2 space-y-2 text-base">
            ...
            {typeof payload.reason === 'string' && (
                <p>{payload.reason === 'user' ? '用户取消' : payload.reason === 'timeout' ? '运行超时' : '原因未记录'}</p>
            )}
...
function HistoricalRunDetail({ runId, onRetry }: { runId: number; onRetry: () => void }) {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                const response = await fetch(`/api/runs/${runId}`, { signal, cache: 'no-store' });
                if (!response.ok) throw new Error('读取历史运行失败');
                const raw: unknown = await response.json();
                signal.throwIfAborted();
                const detail = readRunDetail(raw, String(runId));
                if (detail === null) throw new Error('历史运行响应不符合要求');
        ...
        void load();
        // 取消网络与废弃旧回调同时进行，返回/切换/重试都不会迟到回写。
        return () => { active = false; controller.abort(); };
    }, [runId]);

    if (state.phase === 'loading') return <LoadingPlaceholder compact label="正在读取历史运行详情" />;
    if (state.phase === 'error') return (
        <div className="space-y-3 py-4">
            <p role="alert" className="text-base text-destructive">运行详情读取失败，记录可能已不可访问，请重试。</p>
            <Button type="button" variant="outline" onClick={onRetry}>重试详情</Button>
        </div>
    );
    const { detail } = state;
    return (
        <div className="mt-3 space-y-4">
            <p className="text-base text-muted-foreground">历史记录，只读展示</p>
            {detail.duration_ms !== null && <p className="text-base">总耗时：{detail.duration_ms} ms</p>}
            {detail.events.length === 0 ? <p className="text-base text-muted-foreground">这次运行没有已记录的事件。</p> : (
                <ol aria-label={`运行 ${runId} 的事件`} className="space-y-5 border-l border-border pl-4">
                    {detail.events.map(event => (
                        <li key={event.id} className="min-w-0">
                            <h4 className="break-words text-base font-medium">{eventLabel(event.event_type)}</h4>
                            <time dateTime={event.created_at} title={new Date(event.created_at).toLocaleString('zh-CN')} className="text-base text-muted-foreground">
