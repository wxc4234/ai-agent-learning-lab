...
const components: Components = {
    ...
    pre: ({ children }) => (
        <pre tabIndex={0} className="my-4 max-w-full overflow-x-auto rounded-xl border border-border bg-muted/50 p-4 text-base leading-7 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            {children}
        </pre>
    ),
    table: ({ children }) => (
...
export default memo(function MarkdownMessage({ content }: { content: string }) {
    return (
        <div className="min-w-0 max-w-full text-base leading-8 [overflow-wrap:anywhere] [&>p]:my-3 [&>:first-child]:mt-0 [&>:last-child]:mb-0 [&_h1]:mb-3 [&_h1]:mt-6 [&_h1]:text-2xl [&_h1]:font-semibold [&_h2]:mb-3 [&_h2]:mt-5 [&_h2]:text-xl [&_h2]:font-semibold [&_h3]:mb-2 [&_h3]:mt-4 [&_h3]:font-semibold [&_h4]:font-semibold [&_h5]:font-semibold [&_h6]:font-semibold [&_ul]:my-3 [&_ul]:list-disc [&_ul]:pl-6 [&_ol]:my-3 [&_ol]:list-decimal [&_ol]:pl-6 [&_li]:my-1 [&_li>p]:my-2 [&_blockquote]:my-4 [&_blockquote]:border-l-2 [&_blockquote]:border-border [&_blockquote]:pl-4 [&_blockquote]:text-muted-foreground [&_hr]:my-6 [&_hr]:border-border [&_strong]:font-semibold [&_code]:rounded [&_code]:bg-muted [&_code]:px-1 [&_code]:font-mono [&_code]:text-[0.9em] [&_pre_code]:rounded-none [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_pre_code]:whitespace-pre [&_th]:border-b [&_th]:border-border [&_th]:bg-muted/50 [&_th]:px-3 [&_th]:py-2 [&_td]:border-b [&_td]:border-border/50 [&_td]:px-3 [&_td]:py-2 [&_input]:mr-2">
