...
课程按“一个可验证任务一课”推进，但每一课都默认提供该任务的**完整可运行参考实现**，而不是只给玩具级最小片段。参考实现需覆盖全部相关文件、必要导入、类型、成功/失败测试、运行命令和预期结果，不得用省略号或伪代码隐藏关键逻辑。学习者先理解数据流和设计理由，再参考完整代码亲手实现；只有明确要求代写时，教练才直接修改正式源码。详细教学模板见 [LEARNING_COACH_GUIDE.md](LEARNING_COACH_GUIDE.md)。
...
| 部署、安全、评测与求职 | 10% | Sandbox、Trace、Evals、CI/CD、云部署、作品集和面试表达 |
...
| 第 3 周 | 09-07 ～ 09-13 | 流式 Agent UI + **最小 Agent Runtime**：状态、取消、Tool Registry、参数校验、顺序循环、结构化事件、run_id 与冒烟评测 | 浏览器能观察并取消真实多步骤工具运行；任意运行可按 run_id 还原 | 已提前完成全部验收（当前日历周） |
| 第 4 周 | 09-14 ～ 09-20 | AI 全栈产品底座：认证、授权、Workspace、Task、会话恢复、Redis 幂等/限流、成本与错误摘要 | 用户只能访问自己的 Workspace 与 Task；刷新可恢复运行上下文 | 部分预完成：可观测与预算 |
...
| 第 9 周 | 10-19 ～ 10-25 | **生产化**：后台 Worker、并发控制、Trace、成本路由、评测入 CI、安全、云部署 | 有公网地址；CI 自动评测；任务失败、重启和超预算均可恢复或解释 | 未开始 |
...
> 状态列按整周验收计算：截至 2026-09-10，正式周进度为 3 / 12（25%）。提前实现只记为预完成，不能代替该周其他验收。详细完成项、缺口和下一课以 [LEARNING_HANDOFF.md](LEARNING_HANDOFF.md) 为准。
...
- 登录、Workspace/Task/Run 数据权限和完整审计记录。
...
最终端到端验收不是“聊天能回复”，而是：用户登录并选择 Workspace，提交一个跨文件代码任务；Agent 读取仓库指令和相关代码，生成可见计划，调用受控工具修改文件，必要时等待审批，运行测试，失败后修正，最终展示带引用的说明与可审查 Diff；服务重启或浏览器刷新后仍能恢复任务，整条轨迹可评测、可追踪、可解释成本。
