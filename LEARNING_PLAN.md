...
| 第 5 周 | 09-21 ～ 09-27 | **Coding Tools + Sandbox**：文件、搜索、Shell、Apply Patch、Git Diff、测试、审批策略 | Agent 能在隔离样例仓库完成一次小改动，展示 Diff 并通过测试 | 进行中：路径边界及受限文本读取完成；可信上下文构造完成；执行器透传完成；文件工具适配/过滤完成；local流式入口装配完成；受限目录枚举完成；目录工具适配完成；单文件搜索服务及工具适配完成；只读工具链PC闭环完成；命令请求/结果契约完成；命令输出有界缓冲完成；单路异步排空完成；双路并发排空与收尾完成；命令环境白名单完成；Docker Sandbox最小隔离完成；Docker Sandbox创建参数完成；创建响应/身份解析完成；有界Docker客户端调用已完成；Sandbox创建与身份确认装配已完成；创建结果未确认后的只读核对已完成；已确认未启动容器的显式清理已完成；启动前执行配置复核已完成；Sandbox隔离与资源配置复核已完成；容器停止与停止状态确认已完成；容器启动与启动结果确认已完成；命令退出结果与退出码确认已完成；Docker attach输出帧解析已完成；attach异步读取与收尾已完成；attach HTTP响应读取与校验已完成；attach请求构造已完成；固定socket的attach连接生命周期已完成；启动前订阅与执行收尾协调已完成；已退出容器的显式清理已完成；执行结果到CommandResult的适配已完成；创建、执行与清理的统一编排已完成；失败执行的只读状态核对已完成；受限命令工具适配与安全错误映射已完成；工具定义异步执行支持已完成；Runtime同步/异步工具分派已完成；命令工具内部恢复证据接收已完成；异步工具超时与取消分类已完成；命令恢复记录作用域装配已完成；请求级命令工具注册与能力开放已完成；受限命令工具PC闭环验收已完成；命令结果结构化展示已完成；受限跨目录文件查找已完成；文件查找工具适配与注册已完成；文件查找PC闭环验收已完成；单文件文本替换预览已完成；授权文件预览与基线摘要已完成；文件修改预览工具适配与注册已完成；文件修改预览PC闭环验收已完成；文件修改预览结构化展示已完成；文件修改提案持久化已完成；文件修改提案授权查询已完成；文件修改提案工具适配与注册已完成；文件修改提案PC闭环验收已完成；文件修改提案回执结构化展示已完成；文件修改提案详情HTTP接口已完成；文件修改提案详情BFF代理已完成；文件修改提案详情按需展示已完成；文件修改提案审批状态与决策服务已完成；文件修改提案审批HTTP接口已完成；文件修改提案审批BFF代理已完成；文件修改提案审批交互（当前运行入口）已完成；历史运行提案审阅与审批入口已完成；已批准提案的应用前授权与基线核对已完成；提案应用状态与执行占用已完成；应用占用期间的资源删除与目录变更保护已完成；受限文件替换原语（临时样例范围）已完成；文件替换元数据保护（带明确测试模拟边界）已完成；临时样例中的提案应用执行装配已完成；提案应用状态只读查询已完成；提案应用状态查询HTTP接口及workspace路由整理已完成；提案应用状态查询BFF代理已完成；提案应用状态按需展示已完成；有界扩展属性读取已完成；扩展属性复制能力探测已完成；受限扩展属性复制原语已完成；受限样例应用真实端到端验收完成；登记状态服务/HTTP完成；下一课受限样例登记状态BFF代理 |
...
2026-09-21收尾：第5周推进至文件修改提案详情按需展示，当前只读审阅闭环完成；审批决策和实际写入仍未完成，正式进度保持4/12。今日累计源码/测试/文档归档main，新会话从LEARNING_HANDOFF.md“唯一下一课”接续，默认学习者实现核心。

2026-09-22：元数据保护装配扩展属性复制（临时样例）已完成；临时样例提案应用完整链路验收已完成；提案应用执行结果的公开响应契约已完成；提案应用执行结果的前端解析契约已完成；提案应用执行结果的只读展示组件已完成；提案应用请求契约已完成；服务端临时样例目录生命周期已完成；临时样例进程内登记与失效已完成；临时样例用户任务授权绑定已完成；样例授权门禁与提案执行器装配、受限样例应用HTTP接口及Workspace职责重构已完成（23条新增与386条直接相关共409条通过）；受限样例提案应用BFF代理已完成（新增106条通过）；提案应用客户端请求状态管理已完成（新增25条通过）；受限样例提案应用交互组件已完成（新增14条通过）；受限样例应用组件PC浏览器验收已完成（7组专项、7次模拟请求、无页面错误）；受限样例应用真实端到端验收已完成（真实文件/数据库/重复保护及隔离清理）；受限样例登记状态只读查询已完成（18条新增与23条直接影响共41条通过）；受限样例登记状态HTTP接口已完成（新增20条与相关57条共77条通过）；下一课受限样例登记状态BFF代理，正式进度4/12。
