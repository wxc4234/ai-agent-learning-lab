# 历史验收证据（截至 2026-09-23）

本文件是文档精简时从 ENVIRONMENT.md 搬出的冻结记录，不再追加。各项结果、命令、迁移版本与未验证边界只代表当时的验收；旧全量测试命令不构成当前回归要求。环境操作见 [ENVIRONMENT.md](../../ENVIRONMENT.md)，当前状态只见 [LEARNING_HANDOFF.md](../../LEARNING_HANDOFF.md)。

仅在需要某项历史证据时先搜索标题，再读取命中章节；禁止作为新会话启动上下文全文读取。后续重要阶段证据按 AGENTS.md 记录，不新建逐课日志。

## 早期 Windows 同步与测试隔离记录
...
### 2026-09-23：受限样例来源持久登记

新增`workspace_sample_origins`迁移及模型。创建时在同一数据库事务绑定Workspace目录并登记来源Task/根目录；正常关闭在同一事务撤销两者，确认提交后才清理临时文件。旧Workspace不回填，普通目录无来源。已有内存登记在状态读取、借用和关闭时均核对来源；来源Task删除明确409，BFF及工作台展示可理解的拒绝，不把它当成提交结果未确认。

回归选择：来源字段与绑定提交边界变更→新增迁移/来源测试及既有绑定故障测试；借用门禁与状态核对变更→样例执行、样例状态API及受限apply测试；来源Task删除行为变更→Task删除服务/API、BFF错误映射和工作台浏览器冲突场景。分批结果分别为56 passed（迁移、来源、绑定、Task删除）、42 passed（执行、状态API、受限apply）、新增兄弟Task删除专项1 passed、BFF映射1 passed、受控浏览器冲突场景1 passed。Ruff、定向ESLint、`pnpm typecheck`与`git diff --check`通过；没有运行全量测试。浏览器场景核对一次DELETE、任务与草稿保留和上游私有文本不泄漏，受控409不替代真实后端集成，后端409另由真实隔离PostgreSQL API测试验证。

测试使用独立PostgreSQL库/schema和自有临时目录，结束后自动清理；未访问开发业务表。开发库只执行加表迁移，`alembic current`为`71c43e9a8f02 (head)`，`alembic check`无新增操作；原有项目目录及数据未改。迁移回退只允许来源表为空，以免丢失归属证据。持久记录不恢复进程内句柄或执行权限；进程重启后来源仍在而内存登记丢失，当前状态仍为missing，下一课改为保守的只读诊断。关闭提交成功但回执丢失、或后续文件清理失败时，仍可能留下无法自动处理的现场，不能根据来源表推断文件已经清理。


### 2026-09-23：进程重启后的样例只读诊断

修改`TaskSampleBindings.read_status`：重新授权后无论是否存在内存登记都读取持久来源。无内存登记且Workspace有来源时保守sealed，包括Task或根目录错配；无来源的普通目录仍missing。读取失败直接传播；查询不创建/借用/关闭样例、不提交数据库、不恢复执行权。HTTP、BFF和工作台继续使用既有四状态公开协议。

从`apps/api`运行本课新增及直接受影响的状态服务、来源和HTTP用例（选择依据：仅read_status判定变化→状态/失联/归属/读取失败测试；HTTP响应由其结果直接驱动→对应公开投影测试）：第一批35 passed（3.81s），补充同Workspace兄弟Task错配1 passed（0.24s）。Ruff对修改的Python文件、Node对浏览器脚本的语法检查及`git diff --check`均通过。未重跑全量后端、前端或其他业务领域测试。

隔离真实链路：从仓库根目录设置`STATUS_LOST_REGISTRATION=1`、既有`PLAYWRIGHT_MODULE`和`CHROME_EXECUTABLE`，运行`.venv/bin/python apps/web/test/browser/sample-status-e2e/run.py`。夹具在随机测试库/schema完成真实Alembic升级，由API内部创建自有临时样例并保留原句柄供退出清理，再只在隔离API进程换成空登记；页面仅收到公开标识。Chrome经页面→Next BFF→FastAPI得到来源样例sealed、无来源普通Workspace missing、未知Task 404及非法查询422；没有写请求或页面错误，1366/1920宽度无横向溢出。独立核对example.txt内容/inode/mtime不变，提案仍approved/idle、来源与Workspace目录绑定未被GET改变。结束后原句柄正常关闭，样例目录删除、Workspace解绑、API/Next停止，随机schema/database清理；证据在`apps/web/output/playwright/sample-status-e2e/evidence.json`和同目录截图（忽略的本地产物）。

首次浏览器运行失败于旧夹具把同Workspace的兄弟Task预期为missing；由于Workspace已有来源，该Task按保守错配规则应sealed。把missing场景改为独立的普通Workspace后同一专项重跑通过。该失败未修改生产实现，首轮夹具也执行了资源收尾。浏览器过程模拟登记丢失而非操作系统真实崩溃；不验证崩溃瞬间的文件句柄释放，也不自动处理历史残留目录。


### 2026-09-23：受限样例关闭后的持久清理待办

新增`82d14f5b09ad_add_sample_cleanup_pending.py`及WorkspaceSampleOrigin.lifecycle_state：`active`/`cleanup_pending`，旧来源由数据库默认值保持active；非法值被约束拒绝，待办存在时拒绝迁移回退。关闭样例在第一事务中重新授权并核对活动来源、应用占用，将Workspace.root_path解绑且来源置cleanup_pending，确认提交后才调用原有独占登记的安全文件清理；确认清理完成后在第二短事务撤销来源。第一次提交未知时不碰文件，清理失败/中断/延期或第二次提交失败时进程登记保持封锁。第二次提交回执丢失时数据库可能已删来源，但文件已确认清理；不能从异常反推记录仍在。

待办期间Task删除继续由来源外键/服务门禁拒绝，新的样例bind被已有来源挡住；普通目录bind在归属锁内读取待办并返回既有`workspace_already_bound` 409。授权状态HTTP仍只报sealed，不泄露来源路径或提供自动清理。测试依据：迁移模型变化→新增迁移测试及既有来源迁移测试；关闭的两个提交边界与借用状态变化→新增待办/既有绑定、来源和状态测试；普通目录绑定门禁→服务及HTTP定向用例；公开状态和真实执行关闭链路→状态HTTP、样例执行/应用HTTP定向用例。分批结果：新迁移3 passed，关闭待办核心4 passed，直接影响既有68 passed，新增/相关HTTP与执行8 passed，补充清理中断/延期2 passed。Ruff及`git diff --check`通过；没有前端改动或协议变化，因此未运行前端/浏览器或全量回归。

所有测试仍用随机PostgreSQL测试库/私有schema和自有临时样例，结束自动清理；没有触碰开发业务表。开发库先核对旧修订71c43e9a8f02，再执行加列迁移，`alembic current`为`82d14f5b09ad (head)`，`alembic check`报告无待生成操作。首次组合测试批次因新测试文件漏导入共用`target`夹具而中断，补齐导入后核心专项通过，迁移专项另行完整通过；不是业务实现失败。当前没有清理待办的公开原因或人工核销入口，保留证据不意味着可安全重试文件清理，下一课只做脱敏只读展示。


### 2026-09-23：受限样例清理待办原因的脱敏只读展示

服务`read_status`先按Workspace/Task/Conversation重新授权，再从持久来源给已授权来源Task投影`sealed_reason=cleanup_pending`；同Workspace的其他Task和失联/错配等封锁只得`unavailable`，missing/busy/ready的原因为null。FastAPI严格要求状态与原因组合，BFF和浏览器重新校验并只投影workspace_id/task_id/status/sealed_reason；工作台显示持久清理待办但不推断目录现状，不提供清理或重试。该字段是查询时刻的数据库快照，不是执行权或文件清理许可。

范围依据：状态投影变化→`test_task_sample_status.py`；HTTP公开组合及来源Task隔离→`test_task_sample_status_api.py`；文件清理与第二次提交结果直接决定持久原因→`test_sample_cleanup_pending.py`；BFF/浏览器解析及展示变化→`task-sample-status-route.test.ts`、`sample-status-component`及真实`sample-status-e2e`。隔离PostgreSQL专项52 passed（5.30s），补充“文件已删而待办记录仍在/已消失”的两种提交结果2 passed（0.33s）；前端契约57 passed，定向Ruff、ESLint、TypeScript、Node语法与`git diff --check`通过。初次PostgreSQL及PC组件测试被默认沙箱阻止本机连接，允许本机测试连接后重跑通过；没有运行后端/前端全量回归。

真实链路命令：从仓库根目录设置`STATUS_CLEANUP_PENDING=1`、现有`PLAYWRIGHT_MODULE`与`CHROME_EXECUTABLE`，运行`.venv/bin/python apps/web/test/browser/sample-status-e2e/run.py`。夹具在随机PostgreSQL库/schema应用Alembic后由API内部创建自有样例，真实关闭先提交持久cleanup_pending，测试专用清理函数暂缓物理删除。Chrome经页面→Next BFF→FastAPI读到来源Task的`sealed/cleanup_pending`、同Workspace兄弟Task的`sealed/unavailable`、无来源Workspace的`missing/null`，未知Task为404，非法查询为422；1366/1920 PC视口无横向溢出或页面错误，无写请求。GET前后独立核对样例文件字节/inode/mtime、提案状态、Workspace绑定和来源记录元组均未改变。请求结束后测试夹具才安全关闭自有句柄并删除来源，确认样例目录、来源记录、API/Next及随机库/schema清理；证据见`apps/web/output/playwright/sample-status-e2e/evidence.json`及同目录截图（忽略的本地产物）。首轮浏览器运行在测试脚本等待旧文案时超时，脚本与实际文案同步后同一专项重跑通过；首轮服务和随机库也按夹具收尾。


### 2026-09-23：受限样例清理待办的内部只读处置预检

新增`services/workspace/samples/cleanup_preflight.py`与`TaskSampleBindings.read_cleanup_preflight`，不接HTTP/BFF/UI。先确认local模式及进程身份，在既有进程锁→数据库归属锁顺序下重新授权Workspace/Task/Conversation；同Workspace兄弟Task不是来源Task时固定拒绝。短事务内只读取Workspace绑定与WorkspaceSampleOrigin：无来源为`evidence_missing`，一致活动来源为`not_pending`，待办状态或绑定矛盾为`evidence_inconsistent`。仅一致的cleanup_pending在Session与进程锁退出后取内部root_path，只允许当前服务端临时父目录加`agent-proposal-`和32位小写十六进制名字；从父目录fd用无跟随stat观察候选，不读内容、不借用或关闭登记、不提交数据库。

目录入口缺失只报`directory_missing`，该课当时的候选目录属主/0700形态正确仍报`identity_unverifiable`；链接、普通文件或权限形态异常报`evidence_inconsistent`，父目录/OS读取异常及结构正确但当前TMPDIR已不同的旧路径报`inspection_unavailable`。这些固定分类均不含路径、句柄、令牌或清理许可。该课当时创建时的父/子目录dev/ino只存在进程内，持久来源还没有身份字段；因此当时预检不能比较原记录与候选的数值身份，缺失也不证明清理已完成。数据库与目录是先后两次只读观察，并非原子快照。

范围依据：新增内部方法及候选目录观察→仅运行新增`tests/workspace/samples/test_cleanup_preflight.py`，没有修改现有状态/HTTP/前端行为。命令：在`apps/api`运行`../../.venv/bin/python -m pytest -q -x --tb=short tests/workspace/samples/test_cleanup_preflight.py`；19 passed（1.62s）。覆盖来源/外用户/兄弟Task授权、缺失/活动/矛盾来源、真实待办候选与缺失、同名替换/符号链接/普通文件/权限改变、非法或旧父目录路径、读取与fd关闭故障，以及重复查询前后数据库和文件字节/inode/mtime不变；定向Ruff、compileall与`git diff --check`通过。首次默认沙箱拒绝本机PostgreSQL连接，在允许连接随机隔离测试库后重跑通过；测试结束由共用夹具清理本轮自建样例与库/schema，未触碰开发业务表或运行全量/浏览器回归。


### 2026-09-23：新建受限样例的目录身份持久证据

创建流程从已核验的root_fd与parent_fd取得设备号和inode，登记先复核当前路径，再沿用描述符身份，不将新的路径stat结果当成创建时证据。绑定时在同一数据库事务内写入Workspace.root_path、来源任务及四个身份值；关闭置cleanup_pending时保留证据。93e4c2d7a10b迁移新增四个可空BIGINT，无默认值、不回填旧来源；CHECK仅接受全空或四值完整且有效，降级在锁表后拒绝丢弃非空身份。内部预检先重新授权来源Task并读取持久待办，事务外只读比较当前可信父目录和候选目录：新记录数值匹配为`identity_matches_record`，错配为`evidence_inconsistent`，旧四空仍为`identity_unverifiable`。匹配只是观察时刻的数值相等，inode可能重用，不能据此恢复旧描述符、执行或清理。

范围依据：迁移与模型变化→新增身份迁移及直接受影响的来源/待办迁移用例；创建、登记及绑定身份来源变化→对应真实临时目录与事务用例；预检结果变化→预检成功、旧记录、错配、缺失和故障用例。`apps/api`内运行`../../.venv/bin/python -W error::DeprecationWarning -m pytest -q -x --tb=short`，后接`tests/migrations/test_sample_directory_identity_migration.py`、`test_sample_cleanup_pending_migration.py`、`test_workspace_sample_origin_migration.py`和`tests/workspace/samples/`下的`test_temporary_proposal_sample.py`、`test_temporary_sample_registry.py`、`test_task_sample_binding.py`、`test_sample_origin.py`、`test_sample_cleanup_pending.py`、`test_cleanup_preflight.py`：123 passed（9.23s）。用户指出Pylance不能将`int | None`交给`int()`后，改为逐项运行时检查加`cast`类型收窄，单跑预检匹配用例1 passed（0.22s），无需重跑其余已通过的用例。教练另修复误删的TaskSampleCleanupPreflight类型及重复的root_fd自比较。定向Ruff、compileall和diff check通过；仓库未安装Pyright，未宣称运行了Pyright。无HTTP、BFF、前端协议或浏览器变化，未扩大到其测试或全量回归。

隔离测试使用共用随机PostgreSQL库/schema和自有临时目录，未连接开发业务表；结束后只读查询`pg_database`，`agent_lab_test_`前缀的遗留测试库为0。开发库另先只读确认为82d14f5b09ad、来源记录0条、Workspace 1条，再显式执行加列迁移；完成后`alembic current`为93e4c2d7a10b (head)，`alembic check`为No new upgrade operations detected，`check_database_ready`通过，来源记录仍0条、Workspace仍1条、新身份非空记录0条。开发库未执行降级或业务数据回填。测试对目录身份、文件字节/inode/mtime及数据库查询前后不变有独立断言；本课不证明跨时刻身份不可重用，也不开放自动清理或执行恢复。


### 2026-09-23：清理待办候选目录的描述符级只读复核

本课只改内部`inspect_pending_sample_directory`和直接测试，不改TaskSampleBindings授权、数据库模型或对外协议。实际请求链为可信服务调用`read_cleanup_preflight`→短事务内重新授权来源Task并核对cleanup_pending→退出事务后调用目录观察函数。观察函数先核对当前临时父目录fd与可见路径，再从父fd对规范名字做无跟随stat；形态正确时用`O_RDONLY | O_DIRECTORY | O_NOFOLLOW`打开候选fd，比较打开前目录项、fd的fstat与退出前目录项的设备号/inode、属主和0700权限，并再次核对父目录。候选fd先于父fd关闭，关闭失败也降为`inspection_unavailable`。首次目录项缺失仍报`directory_missing`；已看到目录后被替换、移走或I/O失败都报`inspection_unavailable`；稳定候选与持久身份错配报`evidence_inconsistent`。旧来源四个身份均空时，即使本轮fd核对一致仍报`identity_unverifiable`；新记录稳定数值匹配才报`identity_matches_record`。

范围依据：只改候选目录观察函数→新增候选fd打开标志和关闭、stat→open与open→再次stat之间替换/消失、open/fstat/close故障测试；回归现有真实待办匹配、旧来源未知、持久错配、目录缺失/替换/权限错误、候选读取和父fd关闭等直接受影响用例。`apps/api`运行`../../.venv/bin/python -W error::DeprecationWarning -m pytest -q -x --tb=short tests/workspace/samples/test_cleanup_preflight.py -k 'candidate or legacy_pending or persisted_identity_mismatch or pending_directory_missing or pending_path_replacement or pending_wrong_permissions or pending_filesystem_error or parent_fd_close_error'`，24 passed、9 deselected（1.29s）。定向Ruff、compileall及`git diff --check`通过。测试使用随机隔离PostgreSQL库/schema及真实自有临时目录，断言文件字节/inode/mtime与来源记录前后不变、打开的fd已关闭；结束后只读查询`pg_database`，`agent_lab_test_`前缀遗留库为0。未触碰开发业务表；本课没有迁移，不重复升级或运行Alembic检查，也没有HTTP/BFF/UI变化，不做浏览器或全量回归。

本次描述符只能证明观察期间三次读数一致，不能排除检查完成后的替换、inode重用或ABA；`identity_matches_record`仍不是执行权或清理许可。预检不读取目录正文、不调用registry.borrow/close，也不删除文件。


### 2026-09-23：清理待办只读诊断 HTTP 接口

实际请求链：隔离测试的 TestClient local GET → FastAPI `WorkspaceRoute` 本地边界、路径/查询/正文校验与 `CurrentUser` → `TaskSampleBindings.read_cleanup_preflight` 重新授权 Workspace、Task、Conversation 和持久来源 Task → 短 PostgreSQL 事务读取待办证据 → 事务结束后对自有临时目录执行只读预检 → 严格响应模型仅投影 `workspace_id`、`task_id`、`result`。接口路径为 `/workspaces/{workspace_id}/tasks/{task_id}/sample-cleanup-preflight`。只有七种固定分类可返回；同 Workspace 兄弟 Task、未知和越权资源统一 404 `workspace_not_accessible`，非法路径/查询/正文统一 422 `invalid_sample_cleanup_preflight_input`，读取、内部结果或响应校验失败统一 500 `sample_cleanup_preflight_read_failed`。成功和错误响应均为 `Cache-Control: no-store`，不返回路径、dev/ino、句柄或令牌。`identity_matches_record` 仅为观察时刻的诊断分类，不授予清理、恢复或重试能力。

范围依据：新增 HTTP 投影与输入/故障契约 → 新增 `tests/workspace/samples/test_task_sample_cleanup_preflight_api.py` 全部 32 条；来源 Task 错配改为 404 → 既有 `test_cleanup_preflight.py::test_preflight_requires_owned_source_task[sibling_task]`；共享 `WorkspaceRoute` 请求识别、输入和错误映射 → 既有 `test_task_sample_status_api.py` 中状态成功、非法输入、本地边界和未知故障的 12 条。只运行以上 45 条，没有按目录运行其他后端、前端或浏览器测试。新增专项在 `apps/api` 执行 `../../.venv/bin/python -W error::DeprecationWarning -m pytest -q -x --tb=short tests/workspace/samples/test_task_sample_cleanup_preflight_api.py`，32 passed（3.90s）；既有定向回归 13 passed（2.23s）。相关文件的 Ruff、compileall 与 `git diff --check` 通过。首次默认沙箱阻止连接本机 PostgreSQL；允许连接后两批专项均通过。

真实隔离证据：测试使用共用夹具创建的随机 PostgreSQL 库/私有 schema 和自有临时样例；对新旧身份待办、活动/缺失来源、身份错配和目录缺失做真实读请求。读取前后分别核对 Workspace 绑定与来源元组、样例文件字节及 inode/mtime 不变，禁止读取时创建、借用、关闭登记或提交事务。测试结束后只读查询维护库 `pg_database`，`agent_lab_test_` 前缀遗留库为 0；样例目录由共用测试夹具收尾，未碰开发业务表。本课没有迁移，不执行开发库升级；尚未接入 Next.js BFF、工作台或真实浏览器，也未验证 Windows 文件系统行为。


### 2026-09-23：清理待办只读诊断 BFF 代理

本课新增同源 GET `/api/workspaces/{workspaceId}/tasks/{taskId}/sample-cleanup-preflight`。请求链在受控测试中为直接调用 Next 路由 GET → 独立 BFF 代理验证 local 模式、本机 Host/Origin、路径标识及无查询/正文 → 仅从服务端配置生成 `X-Local-Runtime-Token` → 以 20 秒预算和浏览器取消信号向 FastAPI 同名只读 GET 发起 `cache: no-store` 请求 → 校验 200 正文中的 Workspace/Task 标识与七种固定分类 → 显式重建 `workspace_id`、`task_id`、`result` 三字段响应。成功与错误均设置 `Cache-Control: no-store`；浏览器 Cookie、Authorization、伪造的内部凭证及上游 Set-Cookie/额外字段均不透传。`identity_matches_record` 仍只是诊断快照，不构成清理或恢复权限。

上游 403/404/422 仅保留状态，使用统一固定错误对象；其他非 200 状态、非法 JSON、错资源、非法分类、网络/超时/取消均安全映射为 502，不降级成 `directory_missing` 或 `identity_matches_record`，也不自动重试。非 200 响应体被取消，不读取或回显内部内容。

范围依据：仅新增解析器、BFF 代理、路由和契约测试，未修改现有共享访问模块、现有状态代理或后端协议，因此无直接受影响的既有测试。`apps/web` 运行 `node --experimental-strip-types --test test/features/workspaces/task-sample-cleanup-preflight-route.test.ts`，51 passed（约 0.66s）；覆盖七种分类投影、来源资源匹配、字段过滤、本地访问拒绝、路径/查询/正文限制、上游状态与失败脱敏、错误流关闭、非法 JSON、取消和超时。`pnpm typecheck`（包含 Next 路由类型生成）、四个新增文件的定向 ESLint 与 `git diff --check` 均通过。

测试的上游 `fetch` 全部受控模拟；本课未连接真实 FastAPI、PostgreSQL 或文件系统，也未启动 Next 服务或浏览器，没有本轮外部资源要清理。FastAPI 接口的真实隔离 PostgreSQL/自有目录验证已在上一课完成，本课不重复。工作台按需展示与 PC 只读链路验收留给下一课；未运行全量前端或后端回归。


### 2026-09-23：清理待办只读诊断工作台按需展示与PC只读验收

实际工作台请求链：用户在PC右栏选择Task并点击或按Enter触发“查询清理诊断” → 浏览器同源GET `/api/workspaces/{workspaceId}/tasks/{taskId}/sample-cleanup-preflight` → Next BFF重新校验本地访问与公开响应 → FastAPI重新授权来源Task并读取隔离PostgreSQL中的持久来源 → 数据库事务结束后对自有候选目录做描述符级只读观察 → 浏览器再次校验Workspace/Task与七种固定分类后展示。面板初始不自动请求，只有三个公开字段进入浏览器解析器；失败为未知，不降级成目录缺失或身份匹配。取消、右栏收起、卸载与Task切换先使在途请求失效并拒绝迟到响应。`identity_matches_record`只说明观察时刻数值相等，不授权清理、恢复、重试或文件写入。

范围依据：新增面板与ChatPanel右栏接入 → 新增`sample-cleanup-preflight-component`浏览器专项，直接受影响的既有`sample-status-workbench.mjs`工作台专项；没有修改现有样例状态面板或其他前端业务协议，不运行全量前端/后端测试。组件专项以真实React组件和样式在Chrome运行，使用受控API响应覆盖七种分类、资源错配/非法正文/HTTP与网络失败、键盘触发、手动取消、Task切换和卸载后迟到响应，17次GET、0次写请求、pageerror为0；1366和1920 PC视口无横向溢出。命令：从仓库根目录设置`PLAYWRIGHT_MODULE`和`CHROME_EXECUTABLE`后运行`node apps/web/test/browser/sample-cleanup-preflight-component/run.mjs`。临时组件服务器和编译目录结束后关闭/删除。

直接受影响旧工作台专项使用`BROWSER_APP_MODE=local BROWSER_TEST_SCRIPT=sample-status-workbench.mjs`及上述浏览器环境，运行`.venv/bin/python apps/web/test/browser/run-isolated.py`通过。真实生产工作台的状态与新诊断面板均保持按需GET；收起详情、Task切换清除旧快照，旧状态响应迟到不串入新Task；1366/1920视口无横向溢出、pageerror为0，浏览器写请求0。首次运行新诊断返回404，原因是旧隔离启动器仅复制了样例状态BFF路由；补入新诊断路由后同一专项重跑通过。测试前用于创建Workspace/Task的夹具写请求不计作用户浏览器行为；本轮随机PostgreSQL库/schema和临时服务均已清理。

真实待办专项命令：设置`PLAYWRIGHT_MODULE`与`CHROME_EXECUTABLE`，运行`.venv/bin/python apps/web/test/browser/sample-cleanup-preflight-e2e/run.py`。该夹具用随机隔离PostgreSQL库/schema执行Alembic，FastAPI内部建立自有样例并使来源记录处于cleanup_pending；Next测试页面挂载生产诊断组件，只交给页面公开Workspace/Task标识。真实Chrome→Next BFF→FastAPI返回来源Task的`identity_matches_record`且`Cache-Control: no-store`；同Workspace兄弟Task与未知Task各为404，非法查询为422。请求前后独立比较样例文件字节/inode/mtime、提案状态、Workspace绑定与来源记录元组均未改变；页面发出3次GET、0次写请求，pageerror为0，1366/1920视口无横向溢出。专项结束由测试夹具清理自有样例、来源记录、API/Next服务与随机库/schema，证据见`apps/web/output/playwright/sample-cleanup-preflight-e2e/evidence.json`及同目录截图。首次验证脚本读取组件已主动取消的错误响应正文导致失败，修正为检查状态与未知提示后同一专项重跑通过；首轮资源也经夹具清理。

`pnpm typecheck`、新面板/ChatPanel/直接受影响浏览器脚本的定向ESLint、Node语法检查与`git diff --check`通过。两类浏览器截图已目视检查。无生产后端或数据库迁移改动，未连接开发业务表、模型、Docker或普通项目文件；本课不验证Windows文件系统，也不把只读诊断当作后续操作门禁。
